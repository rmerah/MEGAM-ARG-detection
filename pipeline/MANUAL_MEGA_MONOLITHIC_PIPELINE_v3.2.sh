...
open_file_safe() {
    ...
    if command -v xdg-open > /dev/null 2>&1; then
        ...
        xdg-open "$file_path" 2>/dev/null || log_warn "Impossible d'ouvrir avec xdg-open"
    else
    ...
    if [[ "$download_success" == false ]]; then
        ...
        if conda activate arg_detection 2>/dev/null || conda activate megam_arg 2>/dev/null; then
            if command -v abricate &> /dev/null; then
                ...
                echo "  abricate non trouvé dans cet environnement"
            fi
            conda deactivate 2>/dev/null || true
        else
...
download_mlst_db() {
    ...
    if conda activate assembly_arg 2>/dev/null; then
        ...
        if [[ -d "${CONDA_PREFIX:-}/share/mlst" ]]; then
            ...
            echo "✅ Base MLST copiée depuis conda"
        else
        ...
        echo "❌ Environnement assembly_arg non disponible"
        return 1
    fi
...
find_abricate_dbs() {
    ...
    # Si abricate n'est pas trouvé dans conda, vérifier dans l'environnement actuel
    ...
    # Désactiver l'environnement si on l'a activé
...
setup_abricate_dbs() {
    ...
    # Si abricate n'est toujours pas trouvé, vérifier dans l'environnement actuel
    if [[ "$abricate_found" == false ]]; then
        if command -v abricate &> /dev/null; then
    ...
    if abricate --setupdb 2>&1 | tee /tmp/abricate_setup.log; then
        ...
        if [[ -n "$installed_dbs" ]]; then
            ...
            # Désactiver l'environnement si activé
            if [[ -n "$abricate_env" ]]; then
                conda deactivate 2>/dev/null || true
            fi
...
update_amrfinder_db() {
    ...
    if conda activate arg_detection 2>/dev/null || conda activate annotation_arg 2>/dev/null; then
        ...
        echo "❌ Environnement conda non disponible"
    fi
}
...
update_mlst_db() {
    ...
    # Activer l'environnement
    if conda activate assembly_arg 2>/dev/null; then
        # Copier la base mise à jour depuis conda
        if [[ -d "${CONDA_PREFIX:-}/share/mlst" ]]; then
            ...
            echo "❌ Base MLST non trouvée dans conda"
        fi
        conda deactivate 2>/dev/null || true
    else
        echo "❌ Environnement assembly_arg non disponible"
...
# Fonction pour créer les environnements conda si nécessaire
...
case "$INPUT_TYPE" in
    sra)
        ...
        if [[ -f "$DATA_DIR/${SAMPLE_ID}_1.fastq" ]] || [[ -f "$DATA_DIR/${SAMPLE_ID}_1.fastq.gz" ]]; then
        ...
        elif [[ -f "$DATA_DIR/${SAMPLE_ID}.fastq" ]] || [[ -f "$DATA_DIR/${SAMPLE_ID}.fastq.gz" ]]; then
...
#===============================================================================
# SECTION 11 : VÉRIFICATION ET CRÉATION DES ENVIRONNEMENTS CONDA
#===============================================================================

log_info "═══════════════════════════════════════════════════════════════════"
log_info "VÉRIFICATION ET CRÉATION DES ENVIRONNEMENTS CONDA"
log_info "═══════════════════════════════════════════════════════════════════"

# Créer uniquement les environnements nécessaires selon le type d'entrée
...
log_info "═══════════════════════════════════════════════════════════════════"
log_info "RÉSUMÉ DES ENVIRONNEMENTS"
log_info "═══════════════════════════════════════════════════════════════════"
conda env list | grep -E "^(qc_arg|assembly_arg|annotation_arg|arg_detection|variant_arg|analysis_arg)" || log_warn "Aucun environnement trouvé"
...
if [[ -z "$KRAKEN_DB" ]]; then
    ...
    if [[ "$PROKKA_MODE" == "auto" ]]; then
        extract_species_from_kraken2 "$RESULTS_DIR/01_qc/kraken2/${SAMPLE_ID}_kraken2.report"
    fi
...
# Exécution de Prokka avec les arguments construits
...
if [[ -z "$DETECTED_SPECIES" ]]; then
    ...
    if [[ -f "$KRAKEN_REPORT" ]]; then
        extract_species_from_kraken2 "$KRAKEN_REPORT"
    fi
fi
