...
# Configuration CORS (pour permettre requêtes depuis le frontend)
app.add_middleware(
    CORSMiddleware,
...
async def get_job_results(job_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Erreur lors du parsing des résultats: {str(e)}"
        )
...
async def list_jobs(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Erreur lors de la récupération de la liste des jobs: {str(e)}"
        )
...
async def delete_job(job_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Erreur lors de la suppression du job: {str(e)}"
        )
...
async def delete_all_jobs():
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Erreur lors de la suppression de tous les jobs: {str(e)}"
        )
...
async def stop_job(job_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Erreur lors de l'arrêt du job: {str(e)}"
        )
...
async def list_job_files(job_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Erreur lors du listing des fichiers: {str(e)}"
        )
...
def _run_db_download(db_key: str):
    ...
    try:
        if db_key == "amrfinder":
            _download_with_command(db_key, f"amrfinder_update --force_update --database {db_path}",
                                   fallback_cmd=f"amrfinder --force_update --database {db_path}",
                                   timeout=1800)
            ...
            _download_with_command(db_key, f"git clone https://bitbucket.org/genomicepidemiology/pointfinder_db.git {db_path}",
                                   timeout=600)
            ...
            _download_with_command(db_key, "mlst --update 2>&1 || echo 'MLST update via mlst command'",
                                   timeout=1800)
            ...
            _download_with_command(db_key, f"cd {PIPELINE_DIR} && bash -c 'source {PIPELINE_SCRIPT} && setup_kma_database'",
                                   timeout=1800)
...
def _download_with_wget(db_key: str, url: str, db_path: Path, archive_name: str, extract_cmd: str):
    ...
    try:
        head_result = subprocess.run(
            f"wget --spider --server-response {url} 2>&1 | grep -i content-length | tail -1",
            shell=True, capture_output=True, text=True, timeout=30
        )
        match = re_module.search(r'content-length:\s*(\d+)', head_result.stdout, re_module.IGNORECASE)
        if match:
    ...
    # Étape 2: Lancer wget
    wget_cmd = f"cd {db_path} && wget -O {archive_name} {url}"
    process = subprocess.Popen(wget_cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    extract_result = subprocess.run(
        f"cd {db_path} && {extract_cmd} {archive_name}",
...
def _download_with_command(db_key: str, cmd: str, fallback_cmd: str = None, timeout: int = 1800):
