...
class LaunchAnalysisRequest(BaseModel):
    ...
    def validate_sample_id(cls, v: str) -> str:
        ...
        # Patterns acceptés
        sra_pattern = r'^[SED]RR[0-9]+'
        genbank_pattern = r'^(CP|NC_|NZ_)[0-9]+'
        assembly_pattern = r'^GC[AF]_[0-9]+'

        # Accepter les patterns connus ou un chemin de fichier
        is_valid = (
            re.match(sra_pattern, v) or
            re.match(genbank_pattern, v) or
            re.match(assembly_pattern, v) or
            v.endswith(('.fasta', '.fna', '.fa'))
        )

        if not is_valid:
            raise ValueError(
                f"Format sample_id invalide: '{v}'. "
